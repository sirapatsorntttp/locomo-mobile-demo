...
export const useReserveStore = create<ReserveState>((set, get) => ({
  ...
  addReserve: async (data) => {
    ...
    try {
      ...
      mainStore.addToast("error", `จองรถไม่สำเร็จ: ${e.message}`);
    }
  },
  ...
  bulkAddReserves: async (params) => {
    ...
    for (const employee_id of params.employee_ids) {
      ...
      if (!emp) {
        skipped += params.legs.length * params.travel_dates.length;
        continue;
      }
      ...
      for (const date of params.travel_dates) {
        for (const leg of params.legs) {
          const dir =
            shifts.find((s) => s.id === leg.shift_id)?.trip_direction ??
            "inbound";

          let point_id: string | undefined;
          if (leg.point_mode === "default") {
            point_id =
              emp.transport_defaults?.find((td) => td.trip_direction === dir)
                ?.point_id ?? undefined;
          } else {
    ...
    mainStore.addToast(
      ...
      parts.join(" · ") || "ไม่มีรายการที่ดำเนินการได้",
    );
    mainStore.closeModal();
  ...
  updateReserveState: async (id, state) => {
    const mainStore = useStore.getState();
    try {
      const res = await apiFetch(`/api/reserves/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      set((s) => ({
        reserves: s.reserves.map((r) => (r.id === id ? updated : r)),
      }));
    ...
    try {
      ...
      set((s) => ({
        reserves: s.reserves.map((r) =>
          r.id === id ? { ...r, is_state: "canceled" as ReserveStatus } : r,
        ),
      }));
  ...
  bulkApprove: async (ids) => {
    ...
    try {
      ...
      set((s) => ({
        reserves: s.reserves.map((r) =>
          ids.includes(r.id) && r.is_state === "waiting"
            ? { ...r, is_state: "approved" as ReserveStatus }
            : r,
        ),
      }));
  ...
  bulkCancelReserves: async (ids) => {
    ...
    try {
      await Promise.all(
        ids.map((id) => apiFetch(`/api/reserves/${id}`, { method: "DELETE" })),
      );
      set((s) => ({
        reserves: s.reserves.map((r) =>
          ids.includes(r.id)
            ? { ...r, is_state: "canceled" as ReserveStatus }
            : r,
        ),
      }));
  ...
  bulkEditReserveShift: (ids, shift_id) => {
    ...
    set((s) => ({
      reserves: s.reserves.map((r) =>
        ids.includes(r.id) ? { ...r, shift_id, shift } : r,
      ),
    }));
    mainStore.addToast(
      "success",
      `เปลี่ยนกะเป็น "${shift.name_th}" สำหรับ ${ids.length} รายการสำเร็จ`,
    );
    mainStore.closeModal();
  },
}));
